...
AZ1="us-east-1a"
AZ2="us-east-1b"
KEY_PAIR_NAME="your-key-pair-name"   # Change this to your key pair name
YOUR_IP="$(curl -s https://checkip.amazonaws.com)/32"
AMI_ID="ami-0c02fb55956c7d316"       # Amazon Linux 2 in us-east-1 — update for other regions
...
# ---- 4. NAT Gateway ----
echo "[7/14] Allocating Elastic IP for NAT Gateway..."
EIP_ALLOC_ID=$(aws ec2 allocate-address \
  --domain vpc --region "$AWS_REGION" \
  --query 'AllocationId' --output text)
aws ec2 create-tags --resources "$EIP_ALLOC_ID" --tags Key=Name,Value="${PROJECT_TAG}-eip" Key=Project,Value="$PROJECT_TAG" --region "$AWS_REGION" 2>/dev/null || true
echo "  EIP Allocation: $EIP_ALLOC_ID"

echo "[8/14] Creating NAT Gateway (this takes ~1 minute)..."
...
PRIV_RT_ID=$(aws ec2 create-route-table \
  --vpc-id "$VPC_ID" --region "$AWS_REGION" \
  --query 'RouteTable.RouteTableId' --output text)
aws ec2 create-route --route-table-id "$PRIV_RT_ID" --destination-cidr-block 0.0.0.0/0 --nat-gateway-id "$NAT_GW_ID" --region "$AWS_REGION"
...
BASTION_SG_ID=$(aws ec2 create-security-group \
  ...
  --description "SSH access to bastion from admin IP" \
  --vpc-id "$VPC_ID" --region "$AWS_REGION" \
...
echo "  Bastion SG: $BASTION_SG_ID (SSH allowed from $YOUR_IP)"
...
# ---- 7. Bastion Host ----
echo "[13/14] Launching Bastion Host EC2 instance..."
...
# ---- 8. Save IDs ----
echo "[14/14] Saving resource IDs to $IDS_FILE..."
cat > "$IDS_FILE" <<EOF
VPC_ID=$VPC_ID
...
echo ""
echo " Resource IDs saved to: $IDS_FILE"
echo " Run ./teardown.sh when done to avoid charges."
echo "===================================================="
